...
- **mcp-server.ts**: stdio MCP server，提供 `web_search` 工具。直接 fetch SearXNG JSON API（`/search?format=json`），将结果格式化为 Markdown 返回 Agent。SearXNG 实例作为独立 Docker 容器部署在 Host 上，通过环境变量 `SEARXNG_URL` 配置地址。工具命名空间为 `mcp__searxng__*`。
...
| 网络搜索 | 自托管 SearXNG + stdio MCP | 零成本、多引擎聚合、数据主权、纯工具型无需 Kernel 路由 |
