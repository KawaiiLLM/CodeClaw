...
Agent 调用 MCP 工具（send_message / update_progress）→ KernelClient POST `/api/messages/outbound` → IOBridge 按 channel 查路由表 → 转发到 Skill 对应端点（/send 或 /edit）→ Skill 调用 Telegram API 发送，返回 messageId 沿原路径透传回 Agent。

### Skill 生命周期

容器启动后，SkillServiceManager 扫描 `~/.claude/skills/*/manifest.json`，spawn Skill 子进程并分配端口。Skill 进程启动后自行向内核 `/api/services/register` 注册，IOBridge 建立路由。
...
**Layer 2 — 进度消息（Agent 主动，有 token）**：Agent 调用 `update_progress` MCP 工具，首次发送创建新消息并获得 messageId，后续调用携带 messageId 触发编辑（走 Kernel → Skill /edit）。进度消息带 `progress: true` 标记，Skill 侧跳过 JSONL 写入，不污染聊天记录。
...
| Double-send guard | 闭包 flag | 防止 MCP send + fallback result 重复发送 |
