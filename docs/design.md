...
Agent 调用 MCP 工具（send_message 等）→ KernelClient POST `/api/messages/outbound` → IOBridge 按 channel 查路由表 → 转发到 Skill 对应端点（/send 或 /edit）→ Skill 调用 Telegram API 发送，返回 messageId 沿原路径透传回 Agent。
