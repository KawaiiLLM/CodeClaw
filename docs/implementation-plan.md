...
**CLAUDE.md 内容（核心——Agent 的"自我认知"，L0 常驻层）**：

> CLAUDE.md 是渐进式披露的 L0 层——始终在上下文中，因此必须极度精炼。
> 通道专属细节（MCP 工具用法、数据格式、查询方法）放在各 Skill 的 SKILL.md 中（L2 层），
> SDK 自动在系统提示中列出 Skill 索引，Agent 按需 `read` 加载。
...
遵循 [Agent Skills 开放标准](https://agentskills.io/specification)。YAML frontmatter 提供元数据（SDK 自动注入系统提示的 `<available_skills>` 索引），Markdown body 是 Agent 按需 `read` 的操作手册。

> **设计原则**：SKILL.md 是给 Agent 读的操作手册，不是给人读的安装文档。frontmatter 的 `description` 是 Agent 决定"是否加载"的唯一路由依据。Body 聚焦"怎么用"（MCP 工具、数据格式、查询方法），不含面向运维的配置说明。
>
> **渐进式披露**：SDK 启动时只加载 name + description（~24 tokens/skill），Agent 判断相关时才 `read` 完整 body。这是 Agent 自己管理上下文，不是框架注入。
...
- `react_message` — Add/remove emoji reaction on a message.
- `edit_message` / `delete_message` — Modify or remove messages.
- `get_sticker_set` + `send_sticker` — Browse and send stickers.
- `send_poll` — Create polls.
- `get_message` — Fetch historical messages by date + seq or platformMessageId.

## Chat History
Messages persisted in `~/.claude/data/telegram/<date>/<chatId>.jsonl`.
JSONL fields: seq, ts, tgMsgId, sender, type, text/caption/fileId...

## Message References
reply-to format: `<date>/<chatId>/tgMsgId:<id>`. Resolve via get_message or grep JSONL.

## Group Chat
Only @mentions and replies to bot are forwarded. Use skip_reply when no response needed.
```
...
| SKILL.md 格式 | Agent Skills 开放标准 (YAML frontmatter) | SDK 内置渐进式披露：元数据索引 → Agent 按需 read | Phase 3 |
