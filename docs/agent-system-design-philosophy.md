...
渐进式披露（Progressive Disclosure）已经成为 AI Agent 的上下文管理范式。但我们需要更精确地理解它的本质：**渐进式披露不是框架的优化策略，而是 Agent 自主管理上下文的机制。**

SDK 的内置 Skill 系统实现了三层加载（经 OpenClaw 源码验证，`system-prompt.ts:20-36`）：
...
2. **Skill 知识通过渐进式披露加载** — SKILL.md 的 frontmatter 元数据（name + description）由 SDK 自动注入系统提示，Agent 按需 `read` 完整内容。不是框架注入，是 Agent 自主阅读。
...
├── SKILL.md           # Agent 操作手册（frontmatter: name + description）
├── manifest.json      # 自动启动配置（skillId, entrypoint, type）
...
```yaml
---
name: telegram                    # 必填，kebab-case，与目录名一致
description: "Telegram channel..."  # 必填，Agent 判断"是否加载"的唯一依据
---
...
- 以动词开头写步骤（"使用 send_message 发送消息"），不是散文描述
- 聚焦"怎么用"——MCP 工具调用方式、数据格式、查询方法
- 500 行以内，超出拆到 `references/` 子目录（Tier 3）
- 不含面向运维的配置说明（那属于 config schema 注释）

**description 是路由的关键**：写法应该是触发短语（"Use when handling Telegram conversations or querying chat history"），不是营销文案。description 写得差，Agent 要么永远不加载，要么频繁误加载。
...
5. **文件系统即上下文，Agent 自管理** — 信息存在文件系统，Agent 按需 Read/Grep。Skill 的渐进式披露（元数据索引 → Agent 按需 read 全文）本质上是 Agent 自己管理上下文，而非框架替它决定。框架保证可达，不替 Agent 决定读什么。
...
3. **Skill 包格式规范**：~~SKILL.md 的结构化格式~~ ✅ 已采用 Agent Skills 开放标准（YAML frontmatter: name + description）。config.schema.json 标准和依赖声明规范待定。
